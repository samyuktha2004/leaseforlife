...
export function LevelIndicator({ level }: LevelIndicatorProps) {
  ...
  return (
    <motion.div 
      className="absolute top-6 right-6 flex flex-col items-center gap-3 bg-black/60 backdrop-blur border border-primary/20 p-4 rounded-lg z-40"
    ...
    >
          ...
          >
            ...
            {i < stoneCount && (
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-white font-display font-bold text-sm drop-shadow-[0_0_8px_rgba(0,0,0,0.8)]">
                  {i + 1}
                </span>
              </div>
            )}
      ...
      {level === 5 && (
        <motion.div 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="text-xs text-primary uppercase tracking-widest font-display text-glow mt-2"
        >
          FINAL: Toaster Active
        </motion.div>
      )}
    </motion.div>
  );
